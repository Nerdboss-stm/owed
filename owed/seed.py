"""Seed Stripe test mode with one overdue invoice for a client. Test-data setup, not agent behaviour.
...
INV-0042) stale copies are retagged and the same id is reused, as scripts/seed_stripe.py always did.

Callers: scripts/seed_stripe.py and ui/client_server.py. Nothing in owed/agent imports this.
"""
...
def seed_invoice(client_email: str, *, client_name: Optional[str] = None, invoice_id: Optional[str] = None,
    ...
    for iid, inv in matches:
        if inv.status == "open" and inv.amount_remaining > 0:
            return {"invoice_id": iid, "stripe_id": inv.id, "created": False, "client_email": email,
                    "amount_due": inv.amount_remaining / 100, "due_date": _meta(inv.metadata).get("due_date"),
    ...
    inv = stripe.Invoice.create(
        ...
        metadata={"invoice_id": new_id, "due_date": due.isoformat(), "last_chased_step": "0"},
    )
    ...
    return {"invoice_id": new_id, "stripe_id": inv.id, "created": True, "client_email": email, "client_name": name,
            "amount_due": inv.amount_remaining / 100, "due_date": due.isoformat(), "days_overdue": days_overdue}
